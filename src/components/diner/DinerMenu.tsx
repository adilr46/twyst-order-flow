...
import CheckoutBar from '@/components/diner/CheckoutBar';
...
const DinerMenu: React.FC<DinerMenuProps> = ({ venueSlug }) => {
  ...
  return (
    <>
        ...
        <div className="max-w-4xl mx-auto p-4">
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="pl-10 pr-10 py-2 text-sm border-[#E5E7EB] rounded-[10px] shadow-sm placeholder:text-[#9CA3AF] focus:border-[#1e3a8a] focus:ring-2 focus:ring-[#1e3a8a]/20 transition-all duration-200"
            ...
            {searchQuery && (
              <motion.button
                ...
                className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 w-4 h-4 flex items-center justify-center rounded-full hover:bg-gray-100 transition-all duration-200"
              >
                ×
              </motion.button>
            )}
      ...
      {/* Sticky Checkout Bar */}
      <CheckoutBar cartItemsCount={cart.items.length} isCartOpen={isCartOpen} isProcessing={isProcessing}>
        <StickyCheckoutBar 
          count={cart.items.length}
