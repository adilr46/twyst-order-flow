-- Update order status enum to match new FOH requirements
...
-- Update existing sample order to use new status
UPDATE orders SET status = 'in_prep' WHERE status = 'preparing';